...
export class SolarPanelViewComponent implements OnInit {
  ...
  public production: number | undefined = 0;
  ...
  async updateMeteo() {
    try {
      const response = await fetch(
        `https://wttr.in/${this.city}?format={"meteo":"%c%C","temperature":"%t"}`
      );
  ...
  initUpdateProduction() {
    this.defaultService.getLiveProductionSolarPanels().subscribe((response) => {
      this.production = response.data;
    });
  }

  updateSolarPanelProductionGraphData() {
    this.defaultService.getProductionSolarPanels(1).subscribe((response) => {
  ...
  async ngOnInit() {
    ...
    const locale = new Date().toLocaleDateString('fr-fr', {
      weekday: 'short',
      year: 'numeric',
      month: 'short',
      day: 'numeric',
